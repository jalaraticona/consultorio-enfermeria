...
if(isset($_POST["grabar"])){
	...
	if($dos != ''){
		if(!$u->soloNumero($dos)){
			$mensaje.='Campo dosis no debe superar las 5 dosis. <br>';
		}
	}
	if(!$u->soloLetras($det) or $det == ''){
		$mensaje.='El campo detalle debe ser necesariamente letras. <br>';
	}
	if(!$u->soloNumero($cos) or $cos == ''){
		$mensaje.='El campo costo debe ser numerico. <br>';
	}
	if($mensaje == ''){
		$u = new usuario();
							...
							<tbody>
								...
								<tr>
									...
									<td>
										...
										<?php $tipoo = ['vacuna','proceso enfermero'];
								...
								<tr>
									<td>Nro de dosis: </td>
									<td><input type="number" name="dosis" id="dosis" placeholder="Cantidad de dosis" value="<?php if(!isset($_GET['id'])){echo set_value_input(array(),'dosis','dosis'); } else{echo $dosis;} ?>" /></td>
								</tr>
								<tr>
									<td>Detalle </td>
									<td><input type="text" name="detalle" id="detalle" placeholder="Especifiaciones del servicio" value="<?php if(!isset($_GET['id'])){echo set_value_input(array(),'detalle','detalle'); } else{echo $detalle;} ?>" required="true"/>
									</td>
								</tr>
								<tr>
									<td>Costo:(Expresado en Bs.)</td>
									<td><input type="number" name="costo" id="costo" value="<?php if(!isset($_GET['id'])){echo set_value_input(array(),'costo','costo'); } else{echo $costo;} ?>" required="true"/></td>
								</tr>
								<tr>
